...
def benchmark_buy_and_hold(
    ...
    target_shares = int((initial_cash * target_pct) / buy_price / LOT_SIZE) * LOT_SIZE
    shares = min(target_shares, max_affordable_lot_shares(initial_cash, buy_price))
...
def run_backtest(
    ...
    initial_strategy_target_pct: float = INITIAL_STRATEGY_TARGET_PCT,
    benchmark_target_pct: float = BENCHMARK_TARGET_PCT,
) -> dict[str, Any]:
    ...
    for _, row in df.iterrows():
        ...
        full_hold_benchmark_curve.append(full_hold_benchmark.cash_after_buy + full_hold_benchmark.buy_shares * mark_price)

    strategy_final_asset = equity_curve[-1]
    benchmark_final_asset = benchmark_curve[-1]
    full_hold_benchmark_final_asset = full_hold_benchmark_curve[-1]
    strategy_return = strategy_final_asset / INITIAL_CAPITAL - 1.0
    benchmark_return = benchmark_final_asset / INITIAL_CAPITAL - 1.0
    full_hold_benchmark_return = full_hold_benchmark_final_asset / INITIAL_CAPITAL - 1.0
    ...
    summary = {
        ...
        "full_hold_benchmark_return": round(full_hold_benchmark_return, 8),
        "alpha": round(strategy_return - benchmark_return, 8),
        "alpha_vs_full_hold": round(strategy_return - full_hold_benchmark_return, 8),
        "max_drawdown": round(max_drawdown(equity_curve), 8),
        "benchmark_max_drawdown": round(max_drawdown(benchmark_curve), 8),
        "full_hold_benchmark_max_drawdown": round(max_drawdown(full_hold_benchmark_curve), 8),
        ...
        "benchmark": {
            ...
            "target_pct": benchmark_target_pct,
        },
        "full_hold_benchmark": {
            "buy_time": _timestamp_text(first_row["dt"]),
            "buy_price": full_hold_benchmark.buy_price,
            "buy_shares": full_hold_benchmark.buy_shares,
            "buy_amount": round(full_hold_benchmark.buy_amount, 4),
            "buy_commission": round(full_hold_benchmark.buy_commission, 4),
            "cash_after_buy": round(full_hold_benchmark.cash_after_buy, 4),
            "target_pct": 1.0,
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run 70%-seeded V6 backtest for sz002796.")
    parser.add_argument("--start-date", default=START_DATE)
    parser.add_argument("--end-date", default=None)
    parser.add_argument("--data-dir", default=str(DATA_DIR))
    parser.add_argument("--output-dir", default=str(OUTPUT_DIR))
    parser.add_argument("--initial-strategy-target-pct", type=float, default=INITIAL_STRATEGY_TARGET_PCT)
    parser.add_argument("--benchmark-target-pct", type=float, default=BENCHMARK_TARGET_PCT)
    ...
    summary = run_backtest(
        ...
        initial_strategy_target_pct=args.initial_strategy_target_pct,
    ...
    print(f"alpha_vs_70pct={summary['alpha']:.4%}")
    print(f"alpha_vs_full_hold={summary['alpha_vs_full_hold']:.4%}")
