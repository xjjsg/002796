...
DATA_DIR = Path(__file__).resolve().parent / "data" / "sz002796"
...
def _load_one_file(path: Path, warnings: list[str]) -> pd.DataFrame:
    ...
    jumps = int((df["price"].pct_change().abs() > 0.08).sum())
    if jumps:
        warnings.append(f"{date_str}: {jumps} price jumps above 8% between samples")
