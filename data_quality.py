...
def validate_market_frame(df: pd.DataFrame, source: str = "") -> list[DataQualityIssue]:
    ...
    jumps = int((pct_change > 0.08).sum())
    if jumps:
        issues.append(_issue("warning", f"{source}: {jumps} price jumps above 8% between samples"))

    return issues


class RealtimeDataQualityMonitor:
    def __init__(self, max_price_jump_pct: float = 0.08):
